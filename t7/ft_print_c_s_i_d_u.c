...
/*   Updated: 2023/11/20 20:52:33 by taehkwon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_print_number_d(int n, char *base_arr, int len, int *bytes)
{
	char c;
	long long nb;
	...
	if (*bytes == -1)
		return ;
	if (nb >= len)
		ft_print_number_d(nb / len, base_arr, len, bytes);
	if (*bytes == -1)
		return ;
...
void ft_print_number_x(unsigned int n, char *base_arr, int len, int *bytes)
{
	char c;
	long long nb;
	...
	if (*bytes == -1)
		return ;
	if (nb >= len)
		ft_print_number_x(nb / len, base_arr, len, bytes);
	if (*bytes == -1)
		return ;
